...
function getLocalURL(url) {
  ...
  if (typeof(chrome) !== 'undefined') {
    ...
    return safari.extension.baseURI + 'marker/src' + url;
  }
  else {
    // Mozilla platform.
    ...
    if (url.indexOf(COMMON) === 0) {
      ...
      return 'chrome://marker/' + url.slice(CONTENT.length);
    }
  }
...
// Events fired by marker will have this property set to true.
var MARKDOWN_HERE_EVENT = 'marker-event';
...
var PRIVILEGED_REQUEST_EVENT_NAME = 'marker-request-event';
...
function makeRequestToPrivilegedScript(doc, requestObj, callback) {
  if (typeof(chrome) !== 'undefined') {
    ...
    */
    ...
    requestObj.responseEventName = 'marker-response-event-' + Math.floor(Math.random()*1000000);
