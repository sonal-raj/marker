...
function setupForgotToRenderInterceptors(composeElem, MdhHtmlToText, marked, prefs) {
  ...
  if (!composeSendButton) {
    Utils.consoleLog('marker was unable to find the Gmail "Send" button. Please let the developers know by creating an issue at: https://github.com/sonal-raj/marker/issues');
    return;
  }
...
function probablyWritingMarkdown(mdMaybe, marked, prefs) {
  ...
  */
  ...
  function logMatch(type, match) {
    var log =
      'marker detected unrendered ' + type +
...
function showForgotToRenderPromptAndRespond(composeElem, composeSendButton) {
  ...
  if (typeof(composeElem.ownerDocument.defaultView.openDialog) !== 'undefined') {
    ...
    composeElem.ownerDocument.defaultView.openDialog(
      "chrome://marker/content/confirm-prompt.xul",
      "",
...
function showHTMLForgotToRenderPrompt(html, composeElem, composeSendButton, callback) {
  ...
  Utils.setFocus(composeSendButton.ownerDocument.querySelector('#marker-forgot-to-render-buttons button'));
  ...
  dismissPrompt = function(doc, send) {
    ...
    var forgotToRenderContent = doc.querySelector('#marker-forgot-to-render');
  ...
  closeLink = composeSendButton.ownerDocument.querySelector('#marker-forgot-to-render-close');
  closeLink.addEventListener('click', function(event) {
    eatEvent(event);
    dismissPrompt(event.target.ownerDocument, false);
  });

  backButton = composeSendButton.ownerDocument.querySelector('#marker-forgot-to-render-button-back');
  backButton.addEventListener('click', function(event) {
    eatEvent(event);
    dismissPrompt(event.target.ownerDocument, false);
  });

  sendButton = composeSendButton.ownerDocument.querySelector('#marker-forgot-to-render-button-send');
