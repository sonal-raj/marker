...
/*
 * This file is the heart of Marker. It decides whether we're rendering
...
function findFocusedElem(document) {
  ...
  // details see https://github.com/sonal-raj/marker/issues/31
...
function getOperationalRange(focusedElem) {
  ...
  if (sig) {
    ...
    if (sig.nodeType === sig.ELEMENT_NODE) {
      sig.classList.add('marker-signature');
    }
...
function getMarkdownStylesheet(elem, css) {
  ...
  styleElem.setAttribute('title', 'marker-styles');
  ...
  for (i = 0; i < elem.ownerDocument.styleSheets.length; i++) {
    if (elem.ownerDocument.styleSheets[i].title === 'marker-styles') {
  ...
  if (!stylesheet) {
    throw 'Marker stylesheet not found!';
  }
  ...
  for (i = 0; i < stylesheet.cssRules.length; i++) {
    ...
    for (j = 0; j < selectorMatches.length; j++) {
      ...
      while (elem && (typeof(elem.classList) !== 'undefined')) {
        if (elem.classList.contains('marker-exclude')) {
          elem = 'excluded';
          break;
        }
...
function findMarkdownHereWrapper(focusedElem) {
  ...
  while (wrapper) {
    ...
    if (wrapper.classList && wrapper.classList.contains('marker-wrapper') &&
...
// Finds all Marker wrappers in the given range. Returns an array of the
...
function findMarkdownHereWrappersInRange(range) {
  ...
  cloneWrappers = documentFragment.querySelectorAll('.marker-wrapper[data-md-original]');
  ...
  markdownRenderer(focusedElem, selectedRange, function(mdHtml, mdCss) {
    ...
    mdHtml =
      '<div class="marker-wrapper" ' +
           ...
           'id="marker-wrapper-' + Math.floor(Math.random()*1000000) + '">' +
        mdHtml +
      '</div>';
    ...
    wrapper.ownerDocument.defaultView.setTimeout(function addMutationObserver() {
      ...
      if (typeof(SupportedMutationObserver) !== 'undefined') {
        var observer = new SupportedMutationObserver(function(mutations) {
          wrapper.setAttribute('marker-wrapper-content-modified', true);
          observer.disconnect();
        });
...
function markdownHere(document, markdownRenderer, logger, renderComplete) {
  ...
  // If the cursor (or current selection) is in a Marker wrapper, then
  ...
  if (wrappers && wrappers.length > 0) {
    ...
    for (i = 0; i < wrappers.length; i++) {
      ...
      if (wrappers[i].getAttribute('marker-wrapper-content-modified') &&
          !yesToAll) {
